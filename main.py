...
**********************************************
    ...
    def puede_jugar(self, carta):
        ...
        elif self.lado_oscuro_activo:
            return (carta.color_oscuro == self.color_actual or carta.valor_oscuro == self.valor_actual)
        else:
            return (carta.color_claro == self.color_actual or carta.valor_claro == self.valor_actual)

    def aplicar_efecto_accion(self, carta):
        """ Aplica el efecto de las cartas de acción """
        if carta.tipo_accion == "Toma 1":
            print("El siguiente jugador debe tomar una carta y pierde su turno.")
            self.tomar_cartas(1)
            self.saltar_turno()
            ...
            print("El siguiente jugador debe tomar dos cartas y pierde su turno.")
            self.tomar_cartas(2)
            self.saltar_turno()
        elif carta.tipo_accion == "Toma 5":
            print("El siguiente jugador debe tomar cinco cartas y pierde su turno.")
            self.tomar_cartas(5)
            self.saltar_turno()
        elif carta.tipo_accion == "Salta a todos":
            print("Todos los jugadores pierden su turno, el jugador que tiró la carta juega de nuevo.")
            self.saltar_turno_para_todos()
    ...
    def tomar_cartas(self, cantidad):
        ...
        # Mostrar la carta actual en la mesa en ASCII después de tomar
        carta_actual = self.pila_para_tirar[-1]
        color = carta_actual.color_claro if not self.lado_oscuro_activo else carta_actual.color_oscuro
        valor = carta_actual.valor_claro if not self.lado_oscuro_activo else carta_actual.valor_oscuro
        ascii_carta_actual = f"""
        ╔════════════════════════╗
        ║    {color.upper():^13} ║
        ║                        ║
        ║       {valor:^7}       ║
    ...
    def saltar_turno(self):
        ...
        self.jugador_actual = (self.jugador_actual + (1 if self.sentido_horario else -1)) % len(self.jugadores)

    def saltar_turno_para_todos(self):
        """ Salta el turno de todos los jugadores menos el que jugó la carta """
        self.jugador_actual = (self.jugador_actual + (1 if self.sentido_horario else -1)) % len(self.jugadores)

    def cambiar_color(self):
        """ Cambia el color actual en juego """
        nuevo_color = input("Elige un nuevo color (Azul, Verde, Rojo, Amarillo): ")
